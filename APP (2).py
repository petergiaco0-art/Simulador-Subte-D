...
# (MODIFICACIÓN 2: La función ahora lee desde la URL, no desde un 'uploaded_file')
...
def load_and_clean_data(url_de_datos):
    ...
    # ... (El resto del código de limpieza es IDÉNTICO a antes) ...
    ...
    # --- (MODIFICACIÓN 3: Ahora esta función también calcula Lambda) ---
...
# --- (Función 'calculate_lambda' ya no es necesaria, la fusionamos arriba) ---
# --- (Función 'asignar_direccion' sigue igual) ---
...
# --- (El resto del código de SimPy: CONSTANTES, Clase Anden, Procesos, etc. sigue IDÉNTICO) ---
# ... (copiá y pegá toda la sección 2 y 3 de tu app.py anterior) ...
...
def run_montecarlo_simulation(n_replicaciones, min_freq, max_freq, capacidad, dwell_time, tiempo_viaje, df_lambda, mapas_descenso):
    ...
    for i in range(n_replicaciones):
        semilla_actual = i + 1
        #print(f"Corriendo Réplica {semilla_actual}/{n_replicaciones}...")
...
if df_lambda_final is not None:
    ...
    st.write("Vista previa de la demanda (Tasa Lambda λ):")
    st.dataframe(df_lambda_final.head())

    # 2. Botón para correr la simulación
    if st.button("▶️ Correr Simulación (Escenario Montecarlo)"):
        ...
        st.subheader("Resumen Estadístico (Agregado de Corridas)")
        ...
        st.write("**Gráfico 1: Cola Máxima Promedio por Andén**")
        ...
        st.write("**Gráfico 2: Tiempo de Espera Promedio por Andén**")
